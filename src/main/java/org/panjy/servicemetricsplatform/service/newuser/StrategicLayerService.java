...
import org.panjy.servicemetricsplatform.mapper.message.WechatAccountMapper;
import org.panjy.servicemetricsplatform.mapper.message.WechatActivityMapper;
import org.panjy.servicemetricsplatform.mapper.message.WechatGroupMessageMapper;
...
//    }
    ...
    public double getRetentionRate(Date currentTime, int days) {
        ...
        for (int i = 14; i >= 0; i--) {
            ...
            // 1️⃣ 获取当天新增用户
            List<String> newUsers = getFriendWechatIdsByDate(date);
    ...
    public double getChurnRate(Date currentTime, int days) {
        ...
        // 1. 获取当天新增用户
        List<String> newUsers = getFriendWechatIdsByDate(currentTime);
        logger.info("获取到当天新增用户数: {}", newUsers != null ? newUsers.size() : 0);
    ...
    public Map<String, Object> calculateChurnRateWithGrowth(Date currentDate, int days) {
        ...
        try {
            ...
            // 3. 获取上月同期流失率
            double previousRate = getChurnRate(previousMONTHDate, days);
            ...
            // 5. 构建返回结果
            result.put("currentValue", currentRateBD);
            result.put("previousMONTHValue", previousRateBD);
            result.put("growthRate", growthRate);
            result.put("days", days);
            result.put("currentDate", currentDate);
            result.put("previousMONTHDate", previousMONTHDate);
    ...
    /**
     * 查询传入日期之后所有以一天为单位的对话记录
     *
     * @param startDate 开始日期
     * @return conversation列表
     */
    public List<Conversation> findConversationsByDate(Date startDate) {
        logger.info("开始查询传入日期之后所有以一天为单位的对话记录，开始日期: {}", startDate);
        ...
        // 循环每一天直到当前日期
        while (!currentDay.after(now)) {
            // 设置当天的开始时间（00:00:00）
            currentDay.set(Calendar.HOUR_OF_DAY, 0);
            currentDay.set(Calendar.MINUTE, 0);
            currentDay.set(Calendar.SECOND, 0);
            currentDay.set(Calendar.MILLISECOND, 0);
            Date begin = currentDay.getTime();
            ...
            currentDay.set(Calendar.MILLISECOND, 0);
            Date end = currentDay.getTime();

            logger.info("查询日期范围: {} - {}", begin, end);

            // 调用 Mapper 方法查询当天的记录
            List<Conversation> dailyConversations = wechatMessageMapper.findConversationsByDate(begin, end);
            if (dailyConversations != null && !dailyConversations.isEmpty()) {
                allConversations.addAll(dailyConversations);
                logger.info("查询到 {} 条对话记录", dailyConversations.size());
            }
        ...
        logger.info("总共查询到 {} 条对话记录", allConversations.size());
