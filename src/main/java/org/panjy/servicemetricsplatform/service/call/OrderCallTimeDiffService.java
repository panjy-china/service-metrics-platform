...
import org.panjy.servicemetricsplatform.mapper.call.OrderCallTimeDiffMapper;
...
public class OrderCallTimeDiffService {
     ...
     * @return 首电完成平均用时（天），如果无符合条件的数据则返回0
     */
    public BigDecimal calculateAverageCallCompletionTimeInDaysByMonth(String yearMonth) {
        try {
            // 获取指定月份的订单与首电时间差数据
            List<OrderCallTimeDiff> allTimeDiffs = orderCallTimeDiffMapper.selectOrderCallTimeDiffsByMonth(yearMonth);
            logger.info("获取到指定月份的订单与首电时间差数据记录数: " + allTimeDiffs.size());
            ...
            logger.severe("计算指定月份的首电完成平均用时时发生异常: " + e.getMessage());
            return BigDecimal.ZERO;
        }
    }
    
    /**
     * 计算指定月份的首电完成平均用时环比增长率
     * 
     * @param yearMonth 指定的年月 (格式: yyyy-MM)
     * @return 环比增长率（百分比）
     */
    public double calculateAverageCallCompletionTimeGrowthByMonth(String yearMonth) {
        try {
            // 计算当前月份的平均用时
            BigDecimal currentAverage = calculateAverageCallCompletionTimeInDaysByMonth(yearMonth);
            double currentAverageValue = currentAverage.doubleValue();
            
            // 计算上个月的平均用时
            String previousMonth = getPreviousMonth(yearMonth);
            BigDecimal previousAverage = calculateAverageCallCompletionTimeInDaysByMonth(previousMonth);
            double previousAverageValue = previousAverage.doubleValue();
            
            // 计算环比增长率
            if (previousAverageValue == 0) {
                return currentAverageValue > 0 ? 100.0 : 0.0;
            }
            
            double growthRate = ((currentAverageValue - previousAverageValue) / previousAverageValue) * 100;
            logger.info("计算得到的环比增长率: " + growthRate);
            return growthRate;
        } catch (Exception e) {
            logger.severe("计算指定月份的首电完成平均用时环比增长率时发生异常: " + e.getMessage());
            return 0.0;
        }
    }
