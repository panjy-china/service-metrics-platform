...
public class LLMAnalysisService {
     ...
     * @return 健康画像标签的JSON字符串
     */
    public String extractHealthProfileTags(Conversation conversation) throws Exception {
        StringBuilder sb = new StringBuilder();
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        ...
        String instruction = "你是一名健康画像分析助手，服务于销售场景。\n\n" +
                "请仅基于客户在聊天中自己表达的内容，提取与其身体状况、健康问题或关注点相关的非医疗性标签。\n\n" +
                "提取类别：\n\n" +
                "慢病/长期问题（如高血压、糖尿病）\n\n" +
                "重要医疗史（如心脏支架、胆囊切除）\n\n" +
                "当前/反复不适（如关节疼、睡眠差）\n\n" +
                "指标异常或风险提醒（如“血糖偏高”，需标注来源）\n\n" +
                "健康管理行为（如服药、饮食控制）\n\n" +
                "标签规则：\n\n" +
                "仅当聊天中有依据才生成\n\n" +
                "不推断、不升级风险\n\n" +
                ...
                "{\n" +
                "  \"HealthProfileTags\": {\n" +
                "    \"ChronicConditionsOrLongTermIssues\": [{\"label\": \"...\", \"certainty\": \"...\"}],\n" +
                "    \"SignificantMedicalHistory\": [...],\n" +
                "    \"PhysicalDiscomfort\": [...],\n" +
                "    \"AbnormalIndicatorsOrRiskAlerts\": [...],\n" +
                "    \"HealthManagementBehaviors\": [...]\n" +
                "  },\n" +
                "  \"SalesTips\": [\"...\", \"...\"],\n" +
                "  \"Note\": \"以上标签仅基于客户聊天内容整理，不构成医疗结论\"\n" +
                "}\n\n" +
        ...
        String payload = "{\n" +
                "  \"model\": \"" + QWEN_MODEL + "\",\n" +
                "  \"messages\": [\n" +
                "    {\"role\": \"system\", \"content\": \"你是一名专业的健康画像分析助手，能够准确识别和提取客户聊天中涉及的健康相关信息。\"},\n" +
                    ...
                    try {
                        ...
                        if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
                            ...
                            if (isValidJson(extractedContent)) {
                                ...
                                // 保存分析结果到用户首次反馈表
                                try {
                                    saveAnalysisResultToFeedbackTable(finalResult, conversation.getDate());
                                } catch (Exception e) {
     ...
     */
    private void saveAnalysisResultToFeedbackTable(String analysisResult, LocalDateTime dateTime) {
        try {
        ...
        try {
            ...
            if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
                ...
                if (isValidJson(extractedContent)) {
                    ...
                    // 保存分析结果到用户首次反馈表
                    try {
                        saveAnalysisResultToFeedbackTable(finalResult, userConversation.getDate());
                    } catch (Exception e) {
